...
def _bresenham_nd(const cnp.intp_t[::1] start, const cnp.intp_t[::1] stop):
    """Generate N-D Bresenham line coordinates.
    ...
    Without loss of generality, say the line starts at coordinate (0, 0, 0) (we
    ...
    Let us say we are at position on the line m, n, p where m is the position
    on the major axis.  m is the number of steps we have taken on the major
    axis.

    Call the gradients of the line in each axis g.  g[0] is the gradient of the
    first (here, major) axis, so is 1.

    g[1], g[2] are given by delta[1] / delta[0], delta[2] / delta[0].  All g
    are <= 1.

    With 0, 0, 0 origin, and first (major) axis of m, then (in continuous
    coordinates) n = m g[1] and p = m g[2].

    Let us say that the we have stored the previous position on the line as
    pc[0], pc[1], pc[2].  pc[0] == m - 1.  The question to answer at this step
